    ...
    for line in file:
        ...
        for i in data.get('data'):
            ...
            with open(attrIntoDBFileName,'a',encoding='utf-8') as file:
                file.write('insert into product_category_attribute values('+id+',"'+name+'","'+attrType+'",'+str(isEditable)+','+str(attr_count)+');\n')
            if options is None:
                continue
            else:
            ...
            for j in i.get('options'):
                ...
                with open(attrOptionsIntoDBFileName,'a',encoding='utf-8') as file:
                    options_count+=1
                    file.write('insert into product_category_list values('+str(attr_count)+','+str(options_count)+',"'+optionsName+'");\n')
'''
